...
def train_main_optuna(trial: optuna.Trial):
    ...
    hidden_size = trial.suggest_categorical(
        "hidden_size", [int(128), int(256), int(512)]
    )
    batch_size = trial.suggest_categorical("batch_size", [int(128), int(256), int(512)])
    ...
    learning_rate = trial.suggest_categorical("learning_rate", [4e-3, 4e-5, 4e-4])

    base_params.batch_size = batch_size
    base_params.hidden_size = hidden_size
    base_params.margin = 0.5
    base_params.optimizer = optimizer
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name=f"pykg2vec_{MODEL_NAME}_HPO", direction="maximize", sampler=sampler
    )
    study.optimize(
    ...
    print(study.trials)
    joblib.dump(study, f"pykg2vec_{MODEL_NAME}_STUDY")
